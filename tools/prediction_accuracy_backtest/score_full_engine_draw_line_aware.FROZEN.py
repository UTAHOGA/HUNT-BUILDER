...
def _probability_for_official_row(row: Mapping[str, Any]) -> tuple[float | None, str]:
    ...
    candidate_fields = [metric, "certified_p_draw", "p_draw", "actual_probability", "p_draw_normalized", "p_draw_mean", "predicted_probability", "p_preference_draw", "p_sportsman_draw", "p_bonus_pool", "p_random_pool", "p_availability"]
    seen: set[str] = set()
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    # FIX: scope actuals to target_year to prevent mis-year join (e.g. 2026 pred matched to 2021 Antelope Island)
    # Keep rows where actual_draw_year or model_target_year equals target_year; if neither column present, keep all for backward compat
    def _is_target_year_row(r):
        ay = clean(r.get("actual_draw_year"))
        my = clean(r.get("model_target_year"))
        if not ay and not my:
            return True
        ty = str(args.target_year)
        return ay == ty or my == ty
    filtered_actual_rows = [r for r in actual_rows if _is_target_year_row(r)]
    # If filtering removed everything (truth file without year cols), fall back to unfiltered to avoid zero
    if filtered_actual_rows:
        actual_rows_for_ladders = filtered_actual_rows
    else:
        actual_rows_for_ladders = actual_rows
    ladders, actual_draw_years, actual_model_targets = build_ladders(actual_rows_for_ladders)
