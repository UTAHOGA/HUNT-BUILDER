#!/usr/bin/env python3
"""Audit 2021-for-2022 harvest package ingestion for Hunt Research feeds.

This read-only audit checks the generated 2021 harvest package against
normalized harvest truth, engine-facing harvest feature history, and 2021
draw-result surfaces.
"""
...
PACKAGE_SUPPORT_FILES = [
    "utah_harvest_results_2021_for_2022.sqlite",
    "harvest_results_2021_for_2022_database_report.json",
    "harvest_results_2021_for_2022_database_report.md",
]

FILES = {
    "raw_package_dir": "pipeline/RAW/hunt_unit_database/2022/csv",
...
def build_audit(root: Path) -> tuple[dict[str, object], list[dict[str, object]], list[dict[str, object]], list[dict[str, object]], list[dict[str, object]]]:
    ...
    truth_keys = {feature_key(row) for row in truth_features if feature_key(row)[0] == "2021" and feature_key(row)[1]}
    model_keys = {feature_key(row) for row in model_features if feature_key(row)[0] == "2021" and feature_key(row)[1]}
    truth_by_key = {feature_key(row): row for row in truth_features if feature_key(row)[0] == "2021" and feature_key(row)[1]}
    model_by_key = {feature_key(row): row for row in model_features if feature_key(row)[0] == "2021" and feature_key(row)[1]}
    missing_keys = sorted(truth_keys - model_keys)
    extra_keys = sorted(model_keys - truth_keys)
    missing_rows = [
        {
    ...
    feature_using_2021 = [
        row for row in feature_model if "2021" in clean(row.get("harvest_feature_source_years")).split("|")
    ]
    draw_truth_2021 = summarize_year(draw_truth, "year", "2021")
    draw_v2_2021 = summarize_year(draw_v2, "year", "2021")
    draw_legacy_2021 = summarize_year(draw_legacy, "year", "2021")
    package_report = read_json(raw_dir / "harvest_results_2021_for_2022_database_report.json")
    zip_path = Path("C:/Users/tyler/Desktop/BIBLE HUNT CODES/2021.zip")
    result = "PASS" if not missing_rows else "PASS_WITH_SOURCE_BACKED_ENGINE_FEATURE_GAPS"

    summary = {
        "generated_at_utc": datetime.now(timezone.utc).isoformat(),
        "audit_name": "2021_harvest_database_ingestion",
        "result": result,
        ...
        "harvest_truth_2021": summarize_year(truth_long, "reported_hunt_year", "2021"),
        "harvest_truth_features_2021": summarize_year(truth_features, "reported_hunt_year", "2021"),
        "harvest_model_long_2021": summarize_year(model_long, "reported_hunt_year", "2021"),
        "harvest_model_features_2021": summarize_year(model_features, "reported_hunt_year", "2021"),
        "truth_feature_keys_2021": len(truth_keys),
        "engine_feature_keys_2021": len(model_keys),
        "missing_engine_feature_keys_2021": len(missing_rows),
        "engine_feature_extra_keys_2021": len(extra_rows),
        "feature_model_rows_using_2021": len(feature_using_2021),
        "feature_model_hunt_codes_using_2021": len({clean(row.get("hunt_code")).upper() for row in feature_using_2021 if clean(row.get("hunt_code"))}),
        "draw_truth_2021": draw_truth_2021,
        "draw_reality_engine_v2_2021": draw_v2_2021,
        "draw_reality_engine_legacy_2021": draw_legacy_2021,
        "draw_engine_v2_matches_draw_truth_rows": draw_v2_2021["rows"] == draw_truth_2021["rows"],
        "draw_engine_legacy_has_2021_rows": draw_legacy_2021["rows"] > 0,
        "guardrail": "2021 harvest rows are quality/history inputs only. They are not permit quota truth and are not direct p_draw truth.",
        "recommended_next_step": "No 2021 source-backed harvest feature feeder gaps remain. Continue with the next year audit or downstream regeneration only when the broader production phase calls for it.",
    }
...
def write_markdown(
    ...
    lines = [
        ...
        f"- `2021.zip` exists: `{summary['bible_hunt_codes_zip_exists']}`.",
        f"- Harvest truth rows for 2021: `{summary['harvest_truth_2021']['rows']}`.",
        f"- Harvest truth hunt codes for 2021: `{summary['harvest_truth_2021']['hunt_codes']}`.",
        f"- Engine harvest long rows for 2021: `{summary['harvest_model_long_2021']['rows']}`.",
        f"- Engine harvest feature rows for 2021: `{summary['harvest_model_features_2021']['rows']}`.",
        f"- Missing engine feature rows sourced from normalized truth: `{summary['missing_engine_feature_keys_2021']}`.",
        f"- Engine supplemental feature rows not in normalized truth feature table: `{summary['engine_feature_extra_keys_2021']}`.",
        f"- 2026 feature model rows using 2021 harvest history: `{summary['feature_model_rows_using_2021']}`.",
        f"- Draw truth rows for 2021: `{summary['draw_truth_2021']['rows']}`.",
        f"- Draw truth hunt codes for 2021: `{summary['draw_truth_2021']['hunt_codes']}`.",
        f"- Draw reality engine v2 rows for 2021: `{summary['draw_reality_engine_v2_2021']['rows']}`.",
        f"- Draw reality engine v2 matches draw truth row count: `{summary['draw_engine_v2_matches_draw_truth_rows']}`.",
        f"- Legacy draw_reality_engine.csv has 2021 rows: `{summary['draw_engine_legacy_has_2021_rows']}`.",
        "",
    ...
    lines.extend(
        [
            ...
            "",
            f"`{len(extra_rows)}` engine 2021 feature rows are present beyond the normalized truth feature table.",
            "",
            "## Raw CSV Inventory",
            "",
    ...
    for row in raw_rows:
        lines.append(
            f"| {row['source_csv']} | {row['raw_rows']} | {row['hunt_codes']} | {row['reported_hunt_years']} | {row['model_target_years']} | {row['status']} |"
        )
...
def main() -> int:
    ...
    base = out_dir / "harvest_database_ingestion_2021"
    raw_columns = [
        "source_csv",
        "raw_path",
        "raw_exists",
    ...
    write_csv(out_dir / "harvest_database_ingestion_2021_support_files.csv", support_rows, support_columns)
    write_csv(out_dir / "harvest_database_ingestion_2021_engine_feature_gaps.csv", missing_rows, gap_columns)
    write_csv(out_dir / "harvest_database_ingestion_2021_engine_feature_extras.csv", extra_rows, extra_columns)
