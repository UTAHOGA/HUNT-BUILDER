#!/usr/bin/env python3
"""Audit 2023 harvest database package ingestion for Hunt Research feeds.

This read-only audit checks the generated 2023 all-species harvest database
package against normalized harvest truth, engine-facing harvest feature
history, and 2023 draw-result surfaces.
"""
...
PACKAGE_SUPPORT_FILES = [
    "utah_harvest_results_2023_all_species.sqlite",
    "harvest_results_2023_all_species_database_report.json",
    "harvest_results_2023_all_species_database_report.md",
]

FILES = {
    "raw_package_dir": "pipeline/RAW/hunt_unit_database/2023/harvest_results_2023_all_species_database",
...
def build_audit(root: Path) -> tuple[dict[str, object], list[dict[str, object]], list[dict[str, object]], list[dict[str, object]], list[dict[str, object]]]:
    ...
    truth_keys = {feature_key(row) for row in truth_features if feature_key(row)[0] == "2023" and feature_key(row)[1]}
    model_keys = {feature_key(row) for row in model_features if feature_key(row)[0] == "2023" and feature_key(row)[1]}
    truth_by_key = {feature_key(row): row for row in truth_features if feature_key(row)[0] == "2023" and feature_key(row)[1]}
    model_by_key = {feature_key(row): row for row in model_features if feature_key(row)[0] == "2023" and feature_key(row)[1]}
    ...
    feature_using_2023 = [
        row for row in feature_model if "2023" in clean(row.get("harvest_feature_source_years")).split("|")
    ]
    draw_truth_2023 = summarize_year(draw_truth, "year", "2023")
    draw_v2_2023 = summarize_year(draw_v2, "year", "2023")
    draw_legacy_2023 = summarize_year(draw_legacy, "year", "2023")
    package_report = read_json(raw_dir / "harvest_results_2023_all_species_database_report.json")
    zip_path = Path("C:/Users/tyler/Desktop/BIBLE HUNT CODES/2023.zip")

    result = "PASS" if not missing_rows else "PASS_WITH_SOURCE_BACKED_ENGINE_FEATURE_GAPS"
    summary = {
        "generated_at_utc": datetime.now(timezone.utc).isoformat(),
        "audit_name": "2023_harvest_database_ingestion",
        "result": result,
        ...
        "harvest_truth_2023": summarize_year(truth_long, "reported_hunt_year", "2023"),
        "harvest_truth_features_2023": summarize_year(truth_features, "reported_hunt_year", "2023"),
        "harvest_model_long_2023": summarize_year(model_long, "reported_hunt_year", "2023"),
        "harvest_model_features_2023": summarize_year(model_features, "reported_hunt_year", "2023"),
        "truth_feature_keys_2023": len(truth_keys),
        "engine_feature_keys_2023": len(model_keys),
        "missing_engine_feature_keys_2023": len(missing_rows),
        "engine_feature_extra_keys_2023": len(extra_rows),
        "feature_model_rows_using_2023": len(feature_using_2023),
        "feature_model_hunt_codes_using_2023": len({clean(row.get("hunt_code")).upper() for row in feature_using_2023 if clean(row.get("hunt_code"))}),
        "draw_truth_2023": draw_truth_2023,
        "draw_reality_engine_v2_2023": draw_v2_2023,
        "draw_reality_engine_legacy_2023": draw_legacy_2023,
        "draw_engine_v2_matches_draw_truth_rows": draw_v2_2023["rows"] == draw_truth_2023["rows"],
        "draw_engine_legacy_matches_draw_truth_rows": draw_legacy_2023["rows"] == draw_truth_2023["rows"],
        ...
        "recommended_next_step": "No 2023 source-backed harvest feature feeder gaps remain. Keep supplemental engine feature rows for context unless a later truth review demotes them.",
    }
...
def write_markdown(
    ...
    lines = [
        ...
        f"- `2023.zip` exists: `{summary['bible_hunt_codes_zip_exists']}`.",
        ...
        f"- Engine harvest long rows for 2023: `{summary['harvest_model_long_2023']['rows']}`.",
        f"- Engine harvest feature rows for 2023: `{summary['harvest_model_features_2023']['rows']}`.",
        f"- Missing engine feature rows sourced from normalized truth: `{summary['missing_engine_feature_keys_2023']}`.",
        f"- Engine supplemental feature rows not in normalized truth feature table: `{summary['engine_feature_extra_keys_2023']}`.",
        f"- 2026 feature model rows using 2023 harvest history: `{summary['feature_model_rows_using_2023']}`.",
        ...
        f"- Draw reality engine v2 rows for 2023: `{summary['draw_reality_engine_v2_2023']['rows']}`.",
        f"- Draw reality engine v2 matches draw truth row count: `{summary['draw_engine_v2_matches_draw_truth_rows']}`.",
        f"- Legacy draw_reality_engine.csv matches draw truth row count: `{summary['draw_engine_legacy_matches_draw_truth_rows']}`.",
        "",
    ...
    lines.extend(
        [
            ...
            "",
            f"`{len(extra_rows)}` engine 2023 feature rows are present beyond the normalized truth feature table. These are review-visible supplemental context rows, not missing truth rows.",
            "",
            "## Raw CSV Inventory",
            "",
...
def main() -> int:
    ...
    base = out_dir / "harvest_database_ingestion_2023"
    raw_columns = [
        "source_csv",
        "raw_path",
        "raw_exists",
    ...
    write_csv(out_dir / "harvest_database_ingestion_2023_support_files.csv", support_rows, support_columns)
    write_csv(out_dir / "harvest_database_ingestion_2023_engine_feature_gaps.csv", missing_rows, gap_columns)
    write_csv(out_dir / "harvest_database_ingestion_2023_engine_feature_extras.csv", extra_rows, extra_columns)
