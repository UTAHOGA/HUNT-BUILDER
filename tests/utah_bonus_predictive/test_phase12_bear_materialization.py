...
def test_bear_demand_scenarios_do_not_promote_mean_ladder_guarantee(monkeypatch) -> None:
    """A conditional max-pool result is not a public 100% forecast by itself."""
    from engine.utah_draw_predictive import bear as bear_module

    monkeypatch.setattr(bear_module, "official_bear_draw_odds_hunt_codes", lambda: set())
    monkeypatch.setattr(bear_module, "official_bear_pursuit_hunt_codes", lambda: set())

    def truth_row(code: str, year: int, points: int, eligible: int) -> dict[str, str]:
        return {
            "year": str(year),
            "hunt_code": code,
            "hunt_name": "Unit Test",
    ...
    truth_rows = [
        truth_row("BR9999", 2024, 0, 100),
        truth_row("BR9999", 2024, 6, 8),
        truth_row("BR9999", 2025, 0, 100),
        truth_row("BR9999", 2025, 7, 8),
        truth_row("BR9998", 2024, 6, 8),
        truth_row("BR9998", 2025, 7, 0),
    ]
    db_rows = [
        {
            "hunt_code": "BR9999",
    ...
    rows, report = bear_module.build_bear_bonus_predictions(
        truth_rows=truth_rows,
        db_rows=db_rows,
        forecast_year=2026,
    ...
    target = next(
        row
        for row in rows
        if row.get("hunt_code") == "BR9999" and row.get("residency") == "Resident" and row.get("points") == "8"
    )

    assert float(str(target["p_draw_conditional_mean_ladder"])) == 1.0
    assert 0.0 < float(str(target["p_draw"])) < 1.0
    assert target["historical_demand_scenario_count"] == bear_module.BEAR_DEMAND_SCENARIO_COUNT
    assert "HISTORICAL_DEMAND_SCENARIO_FORECAST" in str(target["data_quality_flags"])
    assert report["mean_ladder_conditional_guarantee_count"] >= 1
