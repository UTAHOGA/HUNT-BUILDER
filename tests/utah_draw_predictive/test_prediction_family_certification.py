...
def test_registry_cannot_hide_failing_residency_slice_with_passing_combined_score(tmp_path: Path) -> None:
    ...
    registry = build_registry(review)
    family = registry["families"]["BONUS_OIL_BIG_GAME"]
    assert family["certification_status"] == EXPERIMENTAL
    assert "RESIDENCY_SLICE_FAILED:Nonresident" in family["failure_reasons"]
    assert registry["residency_acceptance"]["cross_lane_ladder_borrowing_allowed"] is False
...
def test_current_registry_certifies_only_designs_that_pass_coverage_and_metric_gates() -> None:
    ...
    expected_certified = {
        ...
        "BONUS_OIL_BIG_GAME": {"fold_count": 9, "joined_rows": 34_944},
        "BONUS_PLE_BIG_GAME": {"fold_count": 9, "joined_rows": 2_743},
        "PREFERENCE_GENERAL_SEASON_BUCK_DEER": {"fold_count": 9, "joined_rows": 8_936},
    }
