...
let googleEarth3dTriedModes = new Set();
let googleEarth3dModeCycle = ['SATELLITE', 'HYBRID'];
let googleEarth3dLastGmpError = null;
...
function openHuntResearch(huntCode, residency = 'Resident', points = 12, drawPool = 'standard', context = {}) {
  ...
  if (selectedContext.hunt_type) params.set('hunt_type', selectedContext.hunt_type);
  if (selectedContext.draw_design) params.set('draw_design', selectedContext.draw_design);
...
function buildBackpackHuntRecord(hunt, residency = 'Resident', points = 12) {
  ...
  return {
    ...
    draw_design: getDrawDesign(hunt),
...
function getUnitName(h) {
  if (getSpeciesDisplay(h) === 'Cougar' && normalizeHuntCode(getHuntCode(h)) === 'CG9999') return 'Cougar Statewide';
  return firstNonEmpty(h.unitName, h.unit_name, h.UnitName, h.dwr_unit_name, h.DwrUnitName);
}
...
function normalizeHuntTypeLabel(raw) {
  ...
  const lower = value.toLowerCase();
  if (!value) return '';
  if (lower === 'o.t.c.' || lower === 'otc') return 'O.T.C.';
  if (
    lower.includes('harvest objective') ||
    lower.includes('over the counter') ||
    lower.includes('private land only') ||
    lower.includes('private lands only') ||
    lower.includes('pursuit') ||
    lower.includes('availability') ||
    lower.includes('allotment') ||
    lower.includes('remaining permit') ||
    lower.includes('general bull') ||
    lower.includes('spike bull') ||
    lower.includes('spike only')
  ) {
    return 'O.T.C.';
  }
  if (lower.includes('statewide')) return 'Sportsman';
  if (lower.includes('sportsman')) return 'Sportsman';
  if (lower.includes('premium')) return 'Premium Limited Entry';
  if (lower.includes('spot and stalk')) return 'O.T.C.';
  if (lower.includes('management')) return 'Limited Entry';
  if (lower.includes('limited')) return 'Limited Entry';
  if (lower.includes('once-in-a-lifetime') || lower.includes('once in a lifetime')) return 'Once-in-a-lifetime';
  ...
  if (lower.includes('antlerless')) return 'General Season';
...
function getHuntType(h) {
  ...
  const raw = firstNonEmpty(h.hunt_type, h.huntType, h.HuntType, h.type);
...
function normalizeDrawDesignLabel(raw) {
  ...
  if (lower.includes('allocation') || lower.includes('reference only')) return 'Allocation / Reference Only';
  if (lower.includes('allotment')) return 'Capped Permits';
  if (lower.includes('capped')) return 'Capped Permits';
...
function getDrawDesign(h) {
  ...
  const huntCode = safe(getHuntCode(h)).trim().toUpperCase();
  ...
  if (huntCode === 'CG9999' || (species === 'Cougar' && text.includes('unlimited'))) return 'Unlimited';
  if (species === 'Cougar' || huntType.includes('private land only') || huntType.includes('o.t.c') || rawCategory.includes('private land only') || text.includes('allotment')) return 'Capped Permits';
  ...
  if (huntType.includes('otc') || text.includes('over the counter')) return 'Capped Permits';
...
const BLACK_BEAR_HARVEST_OBJECTIVE_ROWS_2026 = [
  { unitName: 'Beaver', objective: 5, seasonGroup: 'Fall', seasonLabel: 'Sept 1 - Oct 25, 2026', methodClass: 'Spot and Stalk', methodNote: 'No dogs/no bait' },
  { unitName: 'Book Cliffs, Bitter Creek/South', objective: 20, seasonGroup: 'Late Summer/Fall', seasonLabel: 'Aug 15 - Oct 1, 2026', methodClass: 'Spot and Stalk', methodNote: 'No dogs/no bait' },
  { unitName: 'Boulder/Kaiparowits', objective: 22, seasonGroup: 'Summer/Fall', seasonLabel: 'July 6-31, 2026 and Sept 1 - Oct 25, 2026', methodClass: 'Spot and Stalk', methodNote: 'No dogs/no bait' },
  { unitName: 'Cache/Ogden', objective: 10, seasonGroup: 'Summer/Fall', seasonLabel: 'July 6-31, 2026 and Sept 1 - Oct 25, 2026', methodClass: 'Spot and Stalk', methodNote: 'No dogs/no bait' },
  { unitName: 'Chalk Creek/East Canyon/Morgan-South Rich', objective: 20, seasonGroup: 'Spring', seasonLabel: 'March 28 - May 25, 2026', methodClass: 'Hounds', methodNote: 'No bait allowed' },
  { unitName: 'Chalk Creek/East Canyon/Morgan-South Rich', objective: 20, seasonGroup: 'Summer/Fall', seasonLabel: 'Aug 1-31, 2026 and Nov 2-8, 2026', methodClass: 'Hounds', methodNote: 'No bait' },
  { unitName: 'Chalk Creek/East Canyon/Morgan-South Rich', objective: 20, seasonGroup: 'Summer/Fall', seasonLabel: 'July 6-31, 2026 and Sept 1 - Oct 25, 2026', methodClass: 'Spot and Stalk', methodNote: 'No dogs/no bait' },
  { unitName: 'Diamond Mtn/Vernal/Bonanza', objective: 9, seasonGroup: 'Fall', seasonLabel: 'Sept 1 - Oct 25, 2026', methodClass: 'Spot and Stalk', methodNote: 'No dogs/no bait' },
  { unitName: 'Fillmore, Pahvant', objective: 1, seasonGroup: 'Fall', seasonLabel: 'Sept 1 - Oct 25, 2026', methodClass: 'Spot and Stalk', methodNote: 'No dogs/no bait' },
  { unitName: 'Fishlake/Thousand Lakes', objective: 5, seasonGroup: 'Fall', seasonLabel: 'Sept 1 - Oct 25, 2026', methodClass: 'Spot and Stalk', methodNote: 'No dogs/no bait' },
  { unitName: 'Kamas/North Slope, Summit', objective: 5, seasonGroup: 'Summer/Fall', seasonLabel: 'July 6-31, 2026 and Sept 1 - Oct 25, 2026', methodClass: 'Spot and Stalk', methodNote: 'No dogs/no bait' },
  { unitName: 'La Sal Mtns', objective: 12, seasonGroup: 'Fall', seasonLabel: 'Sept 1 - Oct 25, 2026', methodClass: 'Spot and Stalk', methodNote: 'No dogs/no bait' },
  { unitName: 'Manti, North', objective: 15, seasonGroup: 'Summer/Fall', seasonLabel: 'July 6-31, 2026 and Sept 1 - Oct 25, 2026', methodClass: 'Spot and Stalk', methodNote: 'No dogs/no bait' },
  { unitName: 'Manti, South/San Rafael, North', objective: 25, seasonGroup: 'Fall', seasonLabel: 'Sept 1 - Oct 25, 2026', methodClass: 'Spot and Stalk', methodNote: 'No dogs/no bait' },
  { unitName: 'Mt Dutton', objective: 2, seasonGroup: 'Fall', seasonLabel: 'Sept 1 - Oct 25, 2026', methodClass: 'Spot and Stalk', methodNote: 'No dogs/no bait' },
  { unitName: 'Nebo', objective: 10, seasonGroup: 'Summer/Fall', seasonLabel: 'July 6-31, 2026 and Sept 1 - Oct 25, 2026', methodClass: 'Spot and Stalk', methodNote: 'No dogs/no bait' },
  { unitName: 'Nine Mile', objective: 25, seasonGroup: 'Spring', seasonLabel: 'March 28 - May 25, 2026', methodClass: 'Hounds', methodNote: 'No bait allowed' },
  { unitName: 'Nine Mile', objective: 10, seasonGroup: 'Fall', seasonLabel: 'Sept 1 - Oct 25, 2026', methodClass: 'Spot and Stalk', methodNote: 'No dogs/no bait' },
  { unitName: 'North Slope, Three Corners/West Daggett', objective: 5, seasonGroup: 'Spring', seasonLabel: 'March 28 - May 25, 2026', methodClass: 'Hounds', methodNote: 'No bait allowed' },
  { unitName: 'North Slope, Three Corners/West Daggett', objective: 3, seasonGroup: 'Fall', seasonLabel: 'Sept 1 - Oct 25, 2026', methodClass: 'Spot and Stalk', methodNote: 'No dogs/no bait' },
  { unitName: 'Panguitch Lake/Zion', objective: 3, seasonGroup: 'Fall', seasonLabel: 'Sept 1 - Oct 25, 2026', methodClass: 'Spot and Stalk', methodNote: 'No dogs/no bait' },
  { unitName: 'San Juan', objective: 12, seasonGroup: 'Fall', seasonLabel: 'Sept 1 - Oct 25, 2026', methodClass: 'Spot and Stalk', methodNote: 'No dogs/no bait' },
  { unitName: 'Wasatch Mtns, Avintaquin/Currant Creek', objective: 15, seasonGroup: 'Spring', seasonLabel: 'March 28 - May 25, 2026', methodClass: 'Hounds', methodNote: 'No bait allowed' },
  { unitName: 'Wasatch Mtns, Avintaquin/Currant Creek', objective: 10, seasonGroup: 'Fall', seasonLabel: 'Sept 1 - Oct 25, 2026', methodClass: 'Spot and Stalk', methodNote: 'No dogs/no bait' },
  { unitName: 'Wasatch Mtns, West-Central', objective: 20, seasonGroup: 'Summer/Fall', seasonLabel: 'July 6-31, 2026 and Sept 1 - Oct 25, 2026', methodClass: 'Spot and Stalk', methodNote: 'No dogs/no bait' },
  { unitName: 'Yellowstone', objective: 10, seasonGroup: 'Spring', seasonLabel: 'March 28 - May 25, 2026', methodClass: 'Hounds', methodNote: 'No bait allowed' },
  { unitName: 'Yellowstone', objective: 10, seasonGroup: 'Fall', seasonLabel: 'Sept 1 - Oct 25, 2026', methodClass: 'Spot and Stalk', methodNote: 'No dogs/no bait' },
];
...
function buildSyntheticConservationPermitHunts(records) {
  ...
  return conservationPermitHuntTable.map((row, index) => {
    ...
    const drawDesign = firstNonEmpty(
      row?.draw_design,
      Array.isArray(row?.organizations) ? row.organizations[0] : '',
      'Conservation',
    );
    ...
    return {
      ...
      huntCategory: drawDesign,
      draw_design: drawDesign,
...
function getSpecificMatrixOptions(selectEl, defaultValue = 'All') {
  if (!selectEl) return [];
  return Array.from(selectEl.options || [])
    .map(option => safe(option.value).trim())
    .filter(value => value && value !== defaultValue);
...
window.selectHuntByKey = (key, options = {}) => {
  ...
  trackAnalytics('hunt_selected', {
    ...
    draw_design: safe(getDrawDesign(h)),
  ...
  panel.innerHTML = `
    <div class="selected-hunt-card">
      <div style="display:grid; gap:10px;">
        ...
        <div><strong>Weapon:</strong> ${weapon}</div>
        <div><strong>Hunt Type:</strong> ${huntType}</div>
        <div><strong>Draw Design:</strong> ${escapeHtml(getDrawDesign(hunt) || 'Not available')}</div>
...
function ensureGoogleEarthFrame() {
  if (googleEarthFrame) return googleEarthFrame;
  const stage = document.querySelector('.map-stage') || document.querySelector('.map-wrap');
  if (!stage) return null;
  let frame = document.getElementById('googleEarthFrame');
  if (!frame) {
    frame = document.createElement('iframe');
    frame.id = 'googleEarthFrame';
    frame.className = 'google-earth-frame';
    frame.title = 'Google Earth view';
    frame.loading = 'lazy';
    frame.setAttribute('referrerpolicy', 'no-referrer-when-downgrade');
    frame.hidden = true;
    stage.appendChild(frame);
  }
  googleEarthFrame = frame;
  return frame;
}
...
function initGoogleEarthFrameEvents() {
  ...
  frame.addEventListener('error', () => {
    updateStatus('Google Earth iframe failed to load. Return to Google map for interactive boundaries.');
  });
}

function normalizeGoogleEarth3dMode(value) {
  const mode = safe(value).trim().toUpperCase();
  if (mode === 'SATELLITE' || mode === 'HYBRID') return mode;
  return '';
}

function buildGoogleEarth3dModeCycle(maps3d) {
  const preferred = normalizeGoogleEarth3dMode(GOOGLE_MAPS_3D_DEFAULT_MODE) || 'SATELLITE';
  const mapMode = maps3d?.MapMode || {};
  const knownModes = [
    normalizeGoogleEarth3dMode(mapMode.SATELLITE),
    normalizeGoogleEarth3dMode(mapMode.HYBRID),
    'SATELLITE',
    'HYBRID',
  ].filter(Boolean);
  const cycle = [preferred, ...knownModes].filter((mode, index, arr) => arr.indexOf(mode) === index);
  return cycle.length ? cycle : ['SATELLITE', 'HYBRID'];
}

function setGoogleEarth3dMode(el, mode) {
  if (!el) return '';
  const normalized = normalizeGoogleEarth3dMode(mode);
  if (!normalized) return '';
  try { el.mode = normalized; } catch (_) {}
  try { el.setAttribute('mode', normalized.toLowerCase()); } catch (_) {}
  return normalized;
}

function toSerializableGoogleEarth3dError(value) {
  ...
  try {
    ...
    Object.keys(value).forEach((key) => {
      ...
      if (next && typeof next === 'object') {
        try {
          out[key] = JSON.parse(JSON.stringify(next));
        } catch (_err) {
          out[key] = String(next);
        }
      } else {
...
function ensureGoogleEarth3dElement() {
  if (USE_GOOGLE_EARTH_IFRAME_MODE) return null;
  const stage = document.querySelector('.map-stage') || document.querySelector('.map-wrap');
  if (!stage) return null;
  ...
  if (!el) {
    ...
    el.setAttribute('mode', normalizeGoogleEarth3dMode(GOOGLE_MAPS_3D_DEFAULT_MODE).toLowerCase() || 'satellite');
    if (safe(GOOGLE_MAPS_3D_MAP_ID).trim()) {
      el.setAttribute('map-id', safe(GOOGLE_MAPS_3D_MAP_ID).trim());
    } else {
      el.removeAttribute('map-id');
    }
    ...
    el.addEventListener('gmp-error', (event) => {
      const currentMode = normalizeGoogleEarth3dMode(el?.mode || el?.getAttribute?.('mode')) || 'UNKNOWN';
      const detailPayload = toSerializableGoogleEarth3dError(event?.detail || null);
      googleEarth3dLastGmpError = {
        mode: currentMode,
        detail: detailPayload,
        message: String(event?.detail?.message || event?.message || '').trim() || null,
        at: new Date().toISOString(),
      };
      const retryMode = googleEarth3dModeCycle.find((mode) => !googleEarth3dTriedModes.has(mode));
      if (retryMode) {
        googleEarth3dTriedModes.add(retryMode);
        try {
          setGoogleEarth3dMode(el, retryMode);
          el.hidden = false;
          updateStatus(`Google Earth 3D retrying in ${retryMode} mode...`);
          return;
        } catch (_) {
      ...
      try {
        console.error('Google Earth 3D gmp-error payload:', {
          ...
          triedModes: Array.from(googleEarth3dTriedModes),
          modeCycle: googleEarth3dModeCycle,
        });
      } catch (_) {}
...
function getWebGlRuntimeDiagnostics() {
  try {
    ...
    return { webgl2, webgl, renderer, vendor };
  } catch (_err) {
    return { webgl2: false, webgl: false, renderer: '', vendor: '' };
  }
}
...
function buildGoogleEarth3dUnavailableMessage(reason) {
  ...
  const mapId = safe(googleEarth3dMap?.mapId || googleEarth3dMap?.getAttribute?.('map-id') || GOOGLE_MAPS_3D_MAP_ID || '').trim();
  const gmpCode = firstNonEmpty(
    googleEarth3dLastGmpError?.detail?.errorCode,
    googleEarth3dLastGmpError?.detail?.code,
    googleEarth3dLastGmpError?.detail?.status,
    ''
  );
  const gmpMsg = firstNonEmpty(
    googleEarth3dLastGmpError?.detail?.message,
    googleEarth3dLastGmpError?.message,
    ''
  );
  return `${reason} ${getGoogleEarth3dTroubleshootingHint()} (mode=${mode}, webgl2=${diag.webgl2}, webgl=${diag.webgl}, mapId=${mapId || 'MISSING'}, gmpCode=${gmpCode || 'NA'}, gmpMessage=${gmpMsg || 'NA'})`.trim();
}
...
function handleGoogleEarth3dUnavailable(reason = 'Google Earth 3D unavailable.') {
  ...
  try {
    console.error('Google Earth 3D unavailable diagnostics:', {
      ...
      gmpError: googleEarth3dLastGmpError,
      triedModes: Array.from(googleEarth3dTriedModes),
      modeCycle: googleEarth3dModeCycle,
    });
  } catch (_) {}
...
async function ensureGoogleEarth3dMap() {
  ...
  googleEarth3dMap = el;
  googleEarth3dLastGmpError = null;
  googleEarth3dModeCycle = buildGoogleEarth3dModeCycle(maps3d);
  googleEarth3dTriedModes = new Set();
  const default3dMode = googleEarth3dModeCycle[0];
  googleEarth3dTriedModes.add(default3dMode);
  setGoogleEarth3dMode(el, default3dMode);
  const mapId = safe(GOOGLE_MAPS_3D_MAP_ID).trim();
  if (mapId) {
    try { el.mapId = mapId; } catch (_) {}
    try { el.setAttribute('map-id', mapId); } catch (_) {}
  }
  // Keep Google Earth native controls enabled.
  el.removeAttribute('default-ui-hidden');
...
function applyMapMode() {
  ...
  if (value === 'earth') {
    ...
    if (USE_GOOGLE_EARTH_IFRAME_MODE) {
...
function getGoogleMapsKeySource() {
  ...
  if (isLocalDevHost()) return 'missing local dev key';
