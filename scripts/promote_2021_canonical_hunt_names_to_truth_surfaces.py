...
AUDIT_DIR = REPO / "audits" / "truth_cross_year" / "final_yearly_canonical_audit" / "2021_for_2022" / "promote_canonical_hunt_names"
...
def lookup_key_fallback(row: dict[str, str], include_points: bool) -> tuple[str, ...]:
    key = [
        row_year(row),
        clean(row.get("source_file")),
        clean(row.get("hunt_code")).upper(),
...
def main() -> None:
    ...
    lookup_long = {}
    lookup_long_fallback = {}
    lookup_hunt = {}
    lookup_hunt_fallback = {}
    lookup_point = {}
    lookup_point_fallback = {}
    for row in candidate_rows:
        if row_year(row) != "2021":
            continue
        name = clean(row.get("hunt_name"))
        lookup_long[lookup_key(row, include_points=True)] = name
        lookup_long_fallback[lookup_key_fallback(row, include_points=True)] = name
        lookup_hunt[lookup_key(row, include_points=False)] = name
        lookup_hunt_fallback[lookup_key_fallback(row, include_points=False)] = name
        if clean(row.get("record_type")).upper() in {"POINT_ROW", "POINT_LEVEL_DRAW_RESULT"}:
            lookup_point[lookup_key(row, include_points=True)] = name
            lookup_point_fallback[lookup_key_fallback(row, include_points=True)] = name

    backups = {}
    for path, label in [
        (DRAW_RESULTS_LONG, "before_promote_2021_long_hunt_names"),
        (FINALIZED_HUNT, "before_promote_2021_finalized_hunt_truth"),
        (FINALIZED_POINT, "before_promote_2021_finalized_point_distribution"),
    ]:
    ...
    for row in long_rows:
        if row_year(row) != "2021":
            continue
        new_name = lookup_long.get(lookup_key(row, include_points=True)) or lookup_long_fallback.get(
            lookup_key_fallback(row, include_points=True)
        )
    ...
    for row in hunt_rows:
        if row_year(row) != "2021":
            continue
        new_name = lookup_hunt.get(lookup_key(row, include_points=False)) or lookup_hunt_fallback.get(
            lookup_key_fallback(row, include_points=False)
        )
        if new_name and clean(row.get("hunt_name")) != new_name:
            row["hunt_name"] = new_name
            hunt_changed += 1
    ...
    for row in point_rows:
        if row_year(row) != "2021":
            continue
        new_name = lookup_point.get(lookup_key(row, include_points=True)) or lookup_point_fallback.get(
            lookup_key_fallback(row, include_points=True)
        )
        if new_name and clean(row.get("hunt_name")) != new_name:
            row["hunt_name"] = new_name
    ...
    report = {
        ...
        "year": 2021,
        "notes": [
            "Promoted cleaned 2021 hunt_name values from the corrected candidate promotion file into canonical master truth surfaces.",
            "Left 2019, 2020, and 2022+ rows unchanged.",
            "Preserved all other columns and source metadata.",
        ],
    }

    (AUDIT_DIR / "promote_2021_canonical_hunt_names_report.json").write_text(
