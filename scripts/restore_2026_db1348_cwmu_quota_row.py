#!/usr/bin/env python3
"""Restore DB1348 as a 2026 CWMU quota/allotment row.
...
the scorable point file and restore it only to quota/canonical display truth.
"""
...
SUMMARY = ROOT / "audits/2026_canonical_reconciliation/restore_2026_db1348_cwmu_quota_row_summary.csv"
...
def quota_row(headers: list[str], db: dict[str, str]) -> dict[str, str]:
    ...
    values = {
        ...
        "notes": "Restored from DATABASE.csv and reviewed CWMU workbook; not inserted into scorable point rows.",
...
def ensure_quota_row(path: Path, db: dict[str, str]) -> tuple[int, int]:
    headers, rows = read_rows(path)
    before = len(rows)
    exists = any(clean(row.get("hunt_code")).upper() == CODE for row in rows)
    if not exists:
        rows.append(quota_row(headers, db))
...
def main() -> int:
    ...
    canonical_before, canonical_after = ensure_quota_row(CANONICAL, db)
    quota_before, quota_after = ensure_quota_row(QUOTA, db)
    ...
    summary = {
        ...
        "quota_rows_before": str(quota_before),
        "quota_rows_after": str(quota_after),
