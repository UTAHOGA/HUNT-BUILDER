#!/usr/bin/env python3
"""Create a Vercel preview that preserves a prior deployment except approved files."""
...
DEFAULT_OVERLAY = (
    ...
    / "processed_data"
    / "audits"
    / "harvest_display_release_20260906"
    / "vercel_overlay"
)
DEFAULT_AUDIT = (
    ROOT
    / "processed_data"
    / "audits"
    / "management_quality_release_20260906"
...
def main() -> int:
    ...
    if actual != APPROVED_FILES:
        raise RuntimeError(f"Overlay file set differs from approval: {sorted(actual)}")

    tree = vercel_api(
        f"/v6/deployments/{args.base_deployment}/files",
        args.scope,
    )
    ...
    missing = APPROVED_FILES - set(by_path)
    if missing:
        raise RuntimeError(f"Approved overlay targets absent from base deployment: {sorted(missing)}")
    ...
    payload = {
        ...
        "meta": {
            "release": "management-quality-display-20260906-safe-preview",
