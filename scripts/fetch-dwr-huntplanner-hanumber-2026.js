...
async function main() {
  ...
  const audit = {
    ...
    rows_with_bucks_per_100_does_objective: rows.filter(r => r.bucks_per_100_does_objective).length,
    rows_with_current_bucks_per_100_does_3yr_average: rows.filter(r => r.current_bucks_per_100_does_3yr_average).length,
    rows_with_bulls_per_100_cows_objective: rows.filter(r => r.bulls_per_100_cows_objective).length,
    rows_with_bulls_per_100_cows_estimate: rows.filter(r => r.bulls_per_100_cows_estimate).length,
    rows_with_total_hunters_previous_hunting_season: rows.filter(r => r.total_hunters_previous_hunting_season).length,
