...
DEFAULT_OUTPUT_DIR = (
    ROOT / "output/pdf/2026_utahdraws_uoga_approved_white_cream_all_species_20260902"
)
...
def build_sections(rows: list[dict[str, str]]) -> list[Section]:
    ...
    for (source_csv, hunt_id, youth_text), group in grouped.items():
        ...
        # DWR's historical youth reports always display Resident and
        # Nonresident panels. The 2026 UtahDraws endpoint omits a panel when
        # that residency has no youth activity, so restore the zero/N/A panel
        # in the PDF only. The retained rows and canonical truth stay intact.
        residencies = (
...
def build_pdf(
    ...
    for page_number, (section, page_rows, totals, part, part_count) in enumerate(pages, start=1):
        art_key = species_art_key(section.species)
        section_art = species_art.get(art_key) if art_key else None
        if section_art is None:
