...
const CATALOG = [
  ...
  {
    key: 'data_hunt_odds_history_json',
    relPath: 'data/hunt_odds_history.json',
    source_type: 'data',
    classification: 'R2_PUBLIC',
    update_mode: 'AUTO_PUBLIC_R2',
    current_storage_mode: 'R2_OBJECT',
    public_use: true,
    served_live: true,
    notes: 'Large site odds-history JSON skipped from pages-dist and served from R2.',
  },
  {
  ...
  {
    key: 'truth_finalized_draw_truth_all_csv',
    relPath: 'data_truth/finalized_draw_truth_ALL.csv',
    source_type: 'data_truth',
    classification: 'R2_PUBLIC',
    update_mode: 'AUTO_ON_TRUTH_REBUILD',
    current_storage_mode: 'R2_OBJECT',
    public_use: true,
    served_live: true,
    notes: 'Unified PDF-derived draw truth artifact; large CSV served from R2, not Git.',
  },
  {
    key: 'truth_finalized_point_distribution_csv',
    relPath: 'data_truth/finalized_point_distribution.csv',
    source_type: 'data_truth',
    classification: 'R2_PUBLIC',
    update_mode: 'AUTO_ON_TRUTH_REBUILD',
    current_storage_mode: 'R2_OBJECT',
    public_use: true,
    served_live: true,
    notes: 'PDF-derived point distribution truth artifact; large CSV served from R2, not Git.',
  },
  {
    key: 'truth_draw_results_long_csv_gz',
    relPath: 'data_truth/draw_results_truth/normalized/draw_results_long.csv.gz',
    source_type: 'data_truth',
    classification: 'R2_PUBLIC',
    update_mode: 'AUTO_ON_PROMOTION',
    current_storage_mode: 'R2_OBJECT',
    ...
    public_use: true,
    served_live: true,
    notes: 'Compressed canonical draw_results_long handoff; gzip R2 object because raw CSV exceeds Wrangler upload size.',
  },
  {
    key: 'truth_engine_input_point_rows_with_totals_csv',
    relPath: 'engine_rebuild_from_truth/engine/draw_engine_input_point_rows_with_totals.csv',
    source_type: 'engine_rebuild_from_truth',
    classification: 'R2_PUBLIC',
    update_mode: 'AUTO_ON_ENGINE_REBUILD',
    current_storage_mode: 'R2_OBJECT',
    public_use: true,
    served_live: true,
    notes: 'Truth-first engine input point rows joined to hunt totals; large CSV served from R2.',
  },
  {
    key: 'truth_engine_input_point_rows_csv',
    relPath: 'engine_rebuild_from_truth/engine/draw_engine_input_point_rows.csv',
    source_type: 'engine_rebuild_from_truth',
    classification: 'R2_PUBLIC',
    update_mode: 'AUTO_ON_ENGINE_REBUILD',
    current_storage_mode: 'R2_OBJECT',
    public_use: true,
    served_live: true,
    notes: 'Truth-first engine input point rows; large CSV served from R2.',
  },
  {
    key: 'truth_engine_prediction_outputs_csv',
    relPath: 'engine_rebuild_from_truth/engine/prediction_outputs.csv',
    source_type: 'engine_rebuild_from_truth',
    classification: 'R2_PUBLIC',
    update_mode: 'AUTO_ON_ENGINE_REBUILD',
    current_storage_mode: 'R2_OBJECT',
    public_use: true,
    served_live: true,
    notes: 'Truth-first rebuilt prediction output CSV; large runtime/download artifact served from R2.',
  },
  {
    key: 'truth_engine_odds_tables_csv',
    relPath: 'engine_rebuild_from_truth/engine/odds_tables.csv',
    source_type: 'engine_rebuild_from_truth',
    classification: 'R2_PUBLIC',
    update_mode: 'AUTO_ON_ENGINE_REBUILD',
    current_storage_mode: 'R2_OBJECT',
    public_use: true,
    served_live: true,
    notes: 'Truth-first rebuilt odds table CSV; large runtime/download artifact served from R2.',
  },
  {
...
function classifyCatalogRows() {
  ...
  for (const entry of CATALOG) {
    ...
    const size = entry.size_bytes || (exists ? fileSize(entry.relPath) : 0);
    const lfsPointer = exists ? isGitLfsPointer(entry.relPath) : false;
    const storageMode = entry.current_storage_mode || (exists ? gitStorageMode(entry.relPath, lfsPointer) : 'MISSING');
    ...
    manifestAssets.push({
      ...
      generated_at: new Date().toISOString(),
      size_bytes: entry.size_bytes || (exists ? size : null),
