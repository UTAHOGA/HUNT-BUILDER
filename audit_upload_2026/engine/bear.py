...
def build_bear_bonus_predictions(
    ...
    if demand_mode not in {"cohort_rollforward", "cumulative_stack"}:
        raise ValueError("Unknown Bear demand mode")
    if demand_mode == "cumulative_stack" and central_estimate_mode != "deterministic":
        ...
        for residency in residencies:
            ...
            if calibration_key not in calibration_cache:
                ...
                calibration_cache[calibration_key] = (
                    _build_retention_and_zero_growth(scoped),
                ...
                row.update(
                    {
                        ...
                        "p_draw_before_existing_ceiling": f"{combine_probabilities(p_bonus_pool, p_random_pool):.6f}",
