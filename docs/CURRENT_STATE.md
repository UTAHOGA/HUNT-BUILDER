...
1. Review the remaining `404` false guarantees and probability-error tails by source-backed draw design and recurring hunt code; correct only identified rule or source-identity defects, then rerun the same source-only adjacent-year review.
2. Resolve the dated Planner/bear transitions. The six no-exact-history codes are now source-classified as intentionally unscored rather than unresolved engine gaps.
