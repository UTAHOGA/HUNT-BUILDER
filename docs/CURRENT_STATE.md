...
The retained official 2018–2022 black-bear PDFs contain separate resident and nonresident point ladders, but the legacy canonicals retain only their combined values. The hash-linked extraction at `data_truth/draw_results_truth/validation/black_bear_2018_2022_pdf_residency_ladders.csv` recombines exactly for each canonical-covered year (`2019`–`2022`), with no value disagreements. The retained 2018 black-bear PDF has likewise been extracted, but its normal canonical does not yet include that bear draw scope; it remains an explicit canonical-freeze task, not a numerical discrepancy. The extraction is validation truth only: no canonical is overwritten and the engine must not use combined-residency history to make resident or nonresident odds. The attempted generic demand-scenario adjustment did not improve the isolated 2021→2022 blind fold and is explicitly not retained.
...
1. Freeze the retained 2018 black-bear PDF scope into the canonical lineage, then add paired source-and-held-out source-lane projections for the exact-recombined 2019–2022 years before rerunning historical bear folds. Do not use a combined-residency row as a resident/nonresident proxy.
