...
Later clarification: the seven 2026 split codes are three new Dolores seasonal
hunts plus four La Sal recodes. Valid predecessor mappings are not permission
to inherit pre-boundary demand. BR7237 and BR7325 are separately new in 2025
and have 2025 draw history. See `docs/bear_demand_forecast_review_2026.md` for
the PDF-verified mappings and completed eight-fold demand-candidate comparisons.
...
## Foundation follow-up and final split-status verification — 2026-09-20

The owner-level foundation and final-classification boundary are now verified.
All seven split codes emit two separate residency records with
`NO_TRANSITION_EVIDENCE`, no source years, blank `forecast_quota_proxy`, and
all probability fields blank. Current whole-hunt references remain separately
labeled: BR7021=2, BR7022=43, BR7126=6, BR7127=27, BR7238=2, BR7239=6,
BR7326=14. They are not inferred R/NR allocations. No source-year awards
exist for those exact post-split codes in 2025; the quota-source label must
not be read as proof that such awards exist.

Fresh generation represents 99 drawing codes / 198 residency combinations
and four correctly named non-draw availability records. Historical product
availability remains unproven where no dated catalog is retained; this does
not convert availability into a draw probability. Current guidance does not
justify counting only four LE hunts or treating pursuit as hunting.

Final evidence resides under
`audits/prediction_release_candidates/bear_program_source_repair_20260919/independent_replay/`.
The isolated crosswalk test mirror passes all four tests and reproduces
96/97/105 inventory rows, 994/1110 permit totals, four lineage recodes, three
new split children and 91 exact matches. The 105-row positive-permit catalog
inventory includes non-public/reference programs; it is not a count of LE
draw hunts. Retained normalized crosswalk/source files are unchanged.
The stale BR7324 test expectation was corrected to verify its actual blank
conservation-reference quota, not to manufacture a permit or probability.

Foundation/PDF/source-boundary regressions: 67 passes. Full predictive suite:
383 passes and the same nine prior failures. All 20 protected files remain
byte-identical. The final five-fold scores, failed certification, two
project-memory failures and 19 broader V3 failures remain as documented in
`docs/bear_controlled_review_2026.md`. No production artifact was rebuilt,
staged, uploaded or deployed.

The preservation claim above is against this work's 20-file baseline. The
older 19-file foundation snapshot predates a saved-ML-CSV change (old hash
`17c3ea08...`, current and availability-baseline hash `90cb4f00...`); its other
18 files match. That historical discrepancy was not introduced or reverted
here and is separately recorded in the controlled-review preservation audit.
