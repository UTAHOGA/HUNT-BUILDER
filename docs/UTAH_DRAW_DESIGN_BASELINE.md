...
Certification reporting preserves the shared `BEAR_DRAW` mechanical key for
source joins, but it evaluates **limited-entry Bear hunting** and **restricted
Bear pursuit** as separate declared populations. General/unlimited pursuit and
harvest-objective availability rows are not probability-score populations.
For legacy frozen forecasts that predate `bear_draw_subtype`, reporting may
hydrate the subtype only from the forecast's retained source-side hunt metadata
through the official Bear classifier; it must not use target-year results to
fill that label.
When the approved source-year canonical retains a generic Bear label, the
hash-linked archived Bear PDF residency-ladder classification is the permitted
historical fallback; code prefixes and permit counts are never subtype proof.
...
- An odd bonus pool gives the extra permit to the max-point side. A one-permit nonresident pool is the documented exception: that permit is issued randomly after the bonus-point round. Retained official Black Bear result ladders demonstrate a Bear-specific one-permit random-draw exception in both residency lanes; this exception is owned only by `bear.py`.
